...
export async function emitPdfFromPagedjsWindow(
    ...
    const pageContainers = Array.from(
        doc.querySelectorAll<HTMLElement>(".paged_page")
    );

    if (pageContainers.length === 0) {
        throw new Error("No paginated pages found in paged-with-floats output");
    }
    ...
    const bundleBase = (
        win as unknown as { __PAGED_BUNDLE_BASE?: string }
    ).__PAGED_BUNDLE_BASE;
    const base = getBaseUrl(options?.baseUrl ?? bundleBase);
...
function parseSourceDocument(
    ...
    const expanded = (sourceHtml ?? "").split("__BASE__").join(baseUrl);
