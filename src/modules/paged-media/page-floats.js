...
class PageFloats extends Handler {
	...
	onDeclaration(declaration, dItem, dList, rule) {
		...
		let selectors = csstree
			...
			.split(",")
			.map((s) => s.trim());
		let side = value && FLOAT_SIDES[value];
		...
		if (value === "left" || value === "right") {
			if (selectors.every((selector) => this.floatReferences[selector])) {
				...
				console.warn(
					"pagedjs: ignoring unsupported combination of float:",
					value,
	...
	afterParsed(parsed) {
		for (let selector in this.pageFloats) {
			...
			try {
				...
				console.warn("pagedjs: invalid page float selector:", selector);
				continue;
			}
		...
		Array.from(parsed.querySelectorAll("[data-page-float]")).forEach(
			(element, index) => {
				element.setAttribute("data-page-float-order", index);
			},
		);
	}
	...
	renderNode(clone) {
		...
		let targets = [];
		if (clone.dataset.pageFloat && !clone.dataset.pageFloatPlaced) {
			targets.push(clone);
		}
		if (clone.querySelectorAll) {
			clone.querySelectorAll("[data-page-float]").forEach((element) => {
		...
		let pageElement = clone.closest(".pagedjs_page");
		if (!pageElement) {
			return;
		}
	...
	beforePageLayout(page) {
		...
		let content = page.element.querySelector(".pagedjs_page_content");
		if (content && !this.flowWrapper(content)) {
	...
	afterOverflowRemoved(removed, rendered) {
		...
		removed.querySelectorAll(".pagedjs_float_spacer").forEach((spacer) => {
			spacer.remove();
		});
		...
		let content = rendered.closest(".pagedjs_page_content");
		if (content) {
			this.syncSpacer(content);
		}
		...
		for (let [ref, entry] of Array.from(this.placed)) {
			...
			if (rolledBack) {
				...
				this.placed.delete(ref);
				let content = entry.element.closest(".pagedjs_page_content");
				if (content) {
					this.syncSpacer(content);
				}
	 ...
	 * @param {Element} pageElement - The .pagedjs_page element.
	 * @param {boolean} skipFitCheck - Place without checking for space.
	 * @param {string} anchorRef - Known anchor ref, from a deferral.
	 * @returns {void}
	 */
	placeFloat(element, side, pageElement, skipFitCheck = false, anchorRef) {
		let content = pageElement.querySelector(".pagedjs_page_content");
		let container = content.querySelector(`.pagedjs_float_${side}`);
		...
		if (oversized) {
			console.warn(
				"pagedjs: page float is taller than the page area; placing anyway",
			);
		}
	...
	findAnchorRef(element) {
		...
		while (
			parent &&
			parent.classList &&
			!parent.classList.contains("pagedjs_page_content")
		) {
	 ...
	 *
	 * @param {Element} content - The .pagedjs_page_content element.
	...
	flowBottom(content) {
		...
		let spacer = wrapper.querySelector(":scope > .pagedjs_float_spacer");
		let hidden = false;
	 ...
	 */
	reservedHeight(content) {
		let container = content.querySelector(".pagedjs_float_bottom");
		if (!container) {
			return 0;
		}
	...
	syncSpacer(content) {
		...
		if (!spacer) {
			...
			spacer.classList.add("pagedjs_float_spacer");
