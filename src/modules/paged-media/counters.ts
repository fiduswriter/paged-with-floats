...
class Counters extends Handler {
	...
	handleIncrement(declaration: CssNode, rule: RuleContext): CounterIncrementRecord[] {
		let increments: CounterIncrementRecord[] = [];
		let children = declaration.value.children;

		children.forEach((data: CssNode, item: List.Cursor) => {
			if (data.type && data.type === "Identifier") {
				...
				let whitespace: List.Cursor | null | undefined, number: List.Cursor | null | undefined, value: number | undefined;
	 ...
	 * @param {Object} rule - The CSS rule node.
	 */
	handleReset(declaration: CssNode, rule: RuleContext) {
		let children = declaration.value.children;

		children.forEach((data: CssNode, item: List.Cursor) => {
			if (data.type && data.type === "Identifier") {
				let name = data.name;
				let whitespace: List.Cursor | null | undefined, number: List.Cursor | null | undefined, value: number | string | undefined;
	 ...
	 */
	processCounters(parsed: DocumentFragment, counters: CountersMap) {
	 ...
	 * @param {Object} counters - The counters map.
	 */
	scopeCounters(counters: CountersMap) {
		let countersArray = [];
		for (let c in counters) {
			if (c !== "page") {
		...
		this.insertRule(
			`.paged_pages { counter-reset: ${countersArray.join(" ")} page 0 pages var(--paged-page-count) footnote var(--paged-footnotes-count) footnote-marker var(--paged-footnotes-count)}`,
		);
	}
	 ...
	 */
	processCounterIncrements(parsed: DocumentFragment, counter: CounterEntry) {
	 ...
	 * @param {Object} counter - The counter object.
	 */
	processCounterResets(parsed: DocumentFragment, counter: CounterEntry) {
	 ...
	 * @param {Object} counter - The counter object.
	 */
	addCounterValues(parsed: DocumentFragment, counter: CounterEntry) {
		...
		let elements = parsed.querySelectorAll<HTMLElement>(
			"[data-counter-" +
				counterName +
				"-reset], [data-counter-" +
				counterName +
		...
		let increment: string | null | undefined, reset: string | null | undefined;
		let resetValue: number | undefined, incrementValue: number | undefined, resetDelta: number;
		let incrementArray: string[];
	 ...
	 */
	addFootnoteMarkerCounter(list: List) {
		let markers: string[] = [];
		csstree.walk(list, {
			visit: "Identifier",
	...
	incrementCounterForElement(element: HTMLElement, incrementArray: string[]) {
		...
		const increments = Array.from(this.styleSheet.cssRules)
			.filter((rule) => {
				...
				return (
					...
					styleRule.style[0] === "counter-increment"
				);
			})
			.map((rule) => (rule as CSSStyleRule).style.counterIncrement);
	...
	mergeIncrements(
		...
		const increments: Record<string, string | number | undefined> = {};
		incrementArray.forEach((increment) => {
			let values = increment.split(" ");
			for (let i = 0; i < values.length; i += 2) {
				increments[values[i]] = operator(
				increments[values[i]] as string | undefined,
				values[i + 1],
			);
			}
		});
	...
	afterPageLayout(pageElement: HTMLElement, page: any) {
		let resets: string[] = [];

		let pgreset = pageElement.querySelectorAll<HTMLElement>(
		...
		let notereset = pageElement.querySelectorAll<HTMLElement>(
