...
class Splits extends Handler {
	...
	afterPageLayout(pageElement: HTMLElement, page: any, breakToken: any, chunker: any) {
		...
		let prevPage: Element | undefined;
		...
		let from: HTMLElement | null | undefined; // Capture the last "from" element for alignment handling
		...
		splits.forEach((split) => {
			...
			if (from) {
				...
				if (!from.dataset.splitFrom) {
					from.dataset.splitOriginal = String(true);
				}
			}
		});
	...
	handleAlignment(node: HTMLElement) {
		...
		let align = (styles as unknown as Record<string, string>)["text-align"];
