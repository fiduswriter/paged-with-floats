...
class TargetText extends Handler {
	...
	onContent(funcNode: CssNode, fItem: List.Cursor, fList: List, declaration: DeclarationContext, rule: RuleContext) {
		if (funcNode.name === "target-text") {
			...
			// Replace target-text() with var(--paged-UUID)
			funcNode.name = "var";
	 ...
	 */
	onPseudoSelector(pseudoNode: CssNode, pItem: List.Cursor, pList: List, selector: SelectorContext, rule: RuleContext) {
		rule.ruleNode.block.children.forEach((properties: CssNode) => {
			if (pseudoNode.name === "before" && properties.property === "content") {
				properties.value.children.forEach((prop: CssNode) => {
					if (prop.type === "String") {
